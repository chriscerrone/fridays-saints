...
def _validate_groups(ctx: dict, row_dict: dict):
    ...
    Enforce 'all or none' for grouped controls:
      ...
      - reverb group: reverb_onoff, reverb_channel, reverb_number, reverb_fadetime
    ...
    # --- Reverb group ---
    g_rev = [row_dict["reverb_onoff"], row_dict["reverb_channel"], row_dict["reverb_number"], row_dict["reverb_fadetime"]]
    if any(_present(x) for x in g_rev):
        ...
        _ = _must_be_int(ctx, "reverb fadetime", row_dict["reverb_fadetime"])
...
for idx, r in enumerate(csv_rows):
    ...
    reverb_fadetime  = safe_str(r,16).strip('"')
    ...
    if not any([
        ...
        reverb_channel, reverb_number, reverb_onoff, reverb_fadetime
    ]):
        continue
...
for row in rows:
    ...
    if play in ("on", "off"):
        ...
        try:
            ...
            fadetime = safe_int(row['reverb_fadetime'], 0)
